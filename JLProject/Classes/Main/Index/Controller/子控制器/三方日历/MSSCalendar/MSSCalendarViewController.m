...
//     @{NSFontAttributeName:[UIFont systemFontOfSize:18],NSForegroundColorAttributeName:[AppUtily hexToRGB:@"ffffff"]}];
//    self.navigationItem.hidesBackButton = YES;
//    UIButton *leftBtn = [UIButton buttonWithType:UIButtonTypeCustom];
//    leftBtn.frame  = CGRectMake(0,0,23,23);
//    [leftBtn setImage:[UIImage imageNamed:@"fanghui.png"] forState:UIControlStateNormal];
//    UIBarButtonItem* leftItem = [[UIBarButtonItem alloc]initWithCustomView:leftBtn];
//    self.navigationItem.leftBarButtonItem= leftItem;
//    [leftBtn addTarget:self action:@selector(backToVC:) forControlEvents:UIControlEventTouchUpInside];
//}
//
//
...
//    [_collectionView reloadData];
    ...
    if (calendaItem.month < 10) {
        if (calendaItem.day < 10) {
            str = [NSString stringWithFormat:@"%ld-0%ld-0%ld", calendaItem.year,calendaItem.month,calendaItem.day];
        }else{
            str = [NSString stringWithFormat:@"%ld-0%ld-%ld", calendaItem.year,calendaItem.month,calendaItem.day];
        }
    }else{
        str = [NSString stringWithFormat:@"%ld-%ld-%ld", calendaItem.year,calendaItem.month,calendaItem.day];
    }
    
    NSString *week = @"";
    if (calendaItem.week == 0) {
        week = @"星期日";
    }else if (calendaItem.week == 1) {
        week = @"星期一";

    }else if (calendaItem.week == 2) {
        week = @"星期二";

    }else if (calendaItem.week == 3) {
        week = @"星期三";

    }else if (calendaItem.week == 4) {
        week = @"星期四";

    }else if (calendaItem.week == 5) {
        week = @"星期五";

    }else if (calendaItem.week == 6) {
        week = @"星期六";
    ...
    if([_delegate respondsToSelector:@selector(calendarViewConfirmClickWithStartDate:endDate:)])
    {
        [_delegate calendarViewConfirmClickWithStartDate:str endDate:week];
    }
    [self.navigationController popViewControllerAnimated:YES];
}
