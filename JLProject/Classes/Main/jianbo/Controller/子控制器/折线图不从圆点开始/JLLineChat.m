...
- (void)drawXAndYLineWithContext:(CGContextRef)context{
    ...
    //    CGFloat yPace = (_yLength - _lastYRangeEnd)/(self.yArr.count - 1);
    
    for (NSInteger i = 0; i<self.yArr.count; i++) {
        
        //获得每一个y轴上标注的点的位置
        //        CGPoint p = P_M(self.chartOrigin.x, self.chartOrigin.y - (i+1)*yPace);
        CGPoint p = P_M(self.chartOrigin.x, self.chartOrigin.y - i*_perYlen);
        //获取每一个文字的宽度
    ...
    //    CGFloat xPace = (_xLength-(_lastXRangeEnd+_firstXRangeStart))/(self.xArr.count-1);
    ...
    //绘制x，y轴名称
    CGPoint yTitle = P_M(self.chartOrigin.x + _firstXRangeStart, self.chartOrigin.y - (self.yArr.count * _perYlen));
    //获取每一个文字的宽度
    ...
    CGPoint xTitle = P_M(self.chartOrigin.x + (self.xArr.count * _perXLen) + 10, self.chartOrigin.y - 10);
    //获取每一个文字的宽度
    //    CGFloat yTitlewidth = [self sizeOfStringWithMaxSize:XORYLINEMAXSIZE textFont:self.yDescTextFontSize aimString:_yBiaoZhu].width;
    ...
    for (NSInteger m = 0; m<self.positionArr.count; m++) {
        ...
        for (NSInteger i = 0 ;i< lineDateArr.count;i++ ) {
            ...
            if (_isHavePoint == YES) {
                ...
                [self drawPointWithRedius:2 andColor:[UIColor redColor] andPoint:p andContext:context];
            }else{
            ...
            if (i == 0) {
                ...
                [self drawLineWithContext:context andStarPoint:[lineDateArr[i - 1] CGPointValue] andEndPoint:p andIsDottedLine:NO andColor:positionLineColor];
            }
        }
    }
